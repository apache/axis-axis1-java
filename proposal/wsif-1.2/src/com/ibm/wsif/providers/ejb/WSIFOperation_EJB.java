...
public class WSIFOperation_EJB
    ...
    public WSIFOperation_EJB(
        ...
        try {
            fieldEJBOperationModel =
                (EJBOperation) fieldBindingOperationModel.getExtensibilityElements().get(0);
        } catch (Exception e) {
    ...
    protected Method[] getMethods() throws WSIFException {
        ...
        try {
        	...
        	if (fieldIsHomeInterface) {
                methods = fieldPort.getEjbHome().getClass().getMethods();
            } else {
                methods = fieldPort.getEjbObject().getClass().getMethods();
            }
			...
			{
				if (!methods[i].getName().equals(fieldEJBOperationModel.getMethodName()))
					continue;
    ...
    public boolean executeRequestResponseOperation(
        ...
        try {
			...
			{				
	            if (fieldIsHomeInterface
	                ...
	                {	
	                	...
	                	result = methods[a].invoke(fieldPort.getEjbHome(), compatibleArguments);
	            	...
	            	try
	                {
						...
						if (compatibleArguments == null) break;
						// Parts are compatible so invoke the ctor with the compatible set
	                	result = methods[a].invoke(fieldPort.getEjbObject(), compatibleArguments);
						invokedOK = true;
	                	fieldMethod = methods[a];						
            ...
            throw new WSIFException(
                this +" : Could not invoke '" + fieldEJBOperationModel.getMethodName() + "'",
                ex);
        }
   ...
   public String toString() {
   	 ...
   	 if (fieldPort != null) {
   	 	buff += " wsifPort_EJB:"+fieldPort.toShallowString();
   	 }
   	 else {
   	 	buff += " wsifPort_EJB:null";
   	 }
   	 ...
   	 buff += " EJBOperation:"+fieldEJBOperationModel;
